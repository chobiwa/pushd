...
exports.setupRestApi = function (
  ...
  app.post('/subscribers', authorize('register'), function (req, res) {
    ...
    try {
      ...
      return createSubscriber(fields, (subscriber, created) =>
  ...
  app.get('/subscriber/:subscriber_id', authorize('register'), (req, res) =>
    req.subscriber.get(function (fields) {
      if (fields == null) {
        logger.error(`No subscriber ${req.subscriber.id}`)
      } else {
        logger.verbose(
          `Subscriber ${req.subscriber.id} info: ` + JSON.stringify(fields)
        )
      }
  ...
  app.post(
    ...
    function (req, res) {
      logger.verbose(
        `Setting new properties for ${req.subscriber.id}: ` +
  ...
  app.delete('/subscriber/:subscriber_id', authorize('register'), (req, res) =>
    req.subscriber.delete(function (deleted) {
      if (!deleted) {
        logger.error(`No subscriber ${req.subscriber.id}`)
      }
  ...
  app.get(
    '/subscriber/:subscriber_id/subscriptions',
    authorize('register'),
    (req, res) =>
      req.subscriber.getSubscriptions(function (subs) {
        if (subs != null) {
          const subsAndOptions = {}
          for (let sub of Array.from(subs)) {
            subsAndOptions[sub.event.name] = {
              ignore_message:
                (sub.options & sub.event.OPTION_IGNORE_MESSAGE) !== 0,
            }
          }
          logger.verbose(
            `Status of ${req.subscriber.id}: ` + JSON.stringify(subsAndOptions)
          )
  ...
  app.post(
    ...
    function (req, res) {
      ...
      logger.verbose(
        `Setting subscriptions for ${req.subscriber.id}: ` +
          JSON.stringify(req.body)
      )
      for (let eventId in req.body) {
        ...
        try {
          ...
          if (
            optionsDict != null &&
            typeof optionsDict === 'object' &&
            optionsDict.ignore_message
          ) {
          ...
          error = error1
          logger.error(
            `Failed to set subscriptions for ${req.subscriber.id}: ${error.message}`
          )
      ...
      return req.subscriber.getSubscriptions(function (subs) {
        ...
        logger.verbose(`Event ${req.event.name} info: ` + JSON.stringify(tasks))
        return async.every(
          tasks,
          function (task, callback) {
            let [action, event, options] = Array.from(task)
            if (action === 'add') {
              return req.subscriber.addSubscription(event, options, (added) =>
                callback(added)
              )
            } else if (action === 'del') {
              return req.subscriber.removeSubscription(event, (deleted) =>
                callback(deleted)
              )
            } else if (action === 'set') {
              return req.subscriber.addSubscription(event, options, (added) =>
  ...
  app.get(
    '/subscriber/:subscriber_id/subscriptions/:event_id',
    authorize('register'),
    (req, res) =>
      req.subscriber.getSubscription(req.event, function (options) {
        if (options != null) {
          return res.json({
  ...
  app.post(
    ...
    function (req, res) {
      ...
      return req.subscriber.addSubscription(
        req.event,
        options,
        function (added) {
          if (added != null) {
            // added is null if subscriber doesn't exist
  ...
  app.delete(
    ...
    authorize('register'),
    (req, res) =>
      req.subscriber.removeSubscription(req.event, function (errorDeleting) {
        if (errorDeleting != null) {
          logger.error(
            `No subscriber ${req.subscriber.id} or not subscribed to ${req.event.name}`
          )
        }
  ...
  app.delete('/event/:event_id', authorize('publish'), (req, res) =>
    req.event.delete(function (deleted) {
      if (!deleted) {
        logger.error(`No event ${req.event.name}`)
      }
      if (deleted) {
