...
class PushServiceGCM {
  ...
  push(subscriber, subOptions, payload) {
    return subscriber.get((info) => {
      const messageKey = `${payload.id}-${info.lang || 'int'}-${!!(subOptions !=
      null
        ? subOptions.ignore_message
        : undefined)}`

      // Multicast supports up to 1000 subscribers
      if (
        messageKey in this.multicastQueue &&
        this.multicastQueue[messageKey].tokens.length >= 1000
      ) {
      ...
      if (messageKey in this.multicastQueue) {
        ...
        note.collapseKey =
          payload.event != null ? payload.event.name : undefined
        if (
        ...
        this.multicastQueue[messageKey] = {
          tokens: [info.token],
        ...
        return (this.multicastQueue[messageKey].timeoutId = setTimeout(
          () => this.send(messageKey),
          500
        ))
      }
      ...
      (err, multicastResult) => {
        if (multicastResult == null) {
          return this.logger != null
            ? this.logger.error('GCM Error: empty response')
            : undefined
  ...
  handleResult(result, subscriber) {
    if (result.registration_id != null) {
      ...
      if (error === 'NotRegistered' || error === 'InvalidRegistration') {
        if (this.logger != null) {
          this.logger.warn(
            `GCM Automatic unregistration for subscriber ${subscriber.id}`
          )
        }
        return subscriber.delete()
      } else {
