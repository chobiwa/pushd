...
class PushServiceAPNS {
  ...
  constructor(conf, logger, tokenResolver) {
    ...
    conf.errorCallback = (errCode, note) => {
      return this.logger != null
        ? this.logger.error(`APNS Error ${errCode}: ${note}`)
        : undefined
    }
  ...
  push(subscriber, subOptions, payload) {
    return subscriber.get((info) => {
      ...
      if (
        (subOptions != null ? subOptions.ignore_message : undefined) !== true &&
        (alert = payload.localizedMessage(info.lang))
      ) {
        let title = payload.localizedTitle(info.lang)
        note.alert = title ? { "body": alert, "title": title } : alert
      }
      ...
      let { category, contentAvailable } = payload
      ...
      if (this.payloadFilter != null) {
        for (let key in payload.data) {
          ...
          if (Array.from(this.payloadFilter).includes(key)) {
      ...
      // this.logger.silly(`APNS will send payload ${JSON.stringify(note,null,null)}, ${device}`); // only note.payload will show up!
      ...
      this.driver.send(note, device).then((response) => {
        ...
        response.failed.forEach((failure) => {
          this.logger.error(
            `APNS send Failed for ${JSON.stringify(failure, null, null)}`
          )
        })
      })
