...
class Subscriber {
  ...
  create(redis, fields, cb, tentatives) {
    ...
    if ((fields != null ? fields.proto : undefined) == null) {
      throw new Error("Missing mandatory `proto' field")
    }
    if ((fields != null ? fields.token : undefined) == null) {
      throw new Error("Missing mandatory `token' field")
    }
    ...
    return Subscriber.prototype.getInstanceFromToken(
      redis,
      fields.proto,
      fields.token,
      (subscriber) => {
              ...
              return redis.exists(`subscriber:${id}`, (err, exists) => {
                if (exists) {
                  ...
                  return redis.discard(() => {
                    return Subscriber.prototype.create(
                      redis,
                      fields,
                      cb,
                      tentatives + 1
                  ...
                  fields.created = fields.updated = Math.round(
  ...
  get(cb) {
    ...
    if (this.info != null) {
      ...
      return this.redis.hgetall(this.key, (err, info) => {
        this.info = info
        if ((this.info != null ? this.info.updated : undefined) != null) {
          // subscriber exists
  ...
  addSubscription(event, options, cb) {
    logger.verbose(`Adding subscription`)
    return (
      this.redis
        .multi()
        // check subscriber existance
        ...
        .exec((err, results) => {
          if (results[0] != null) {
            // subscriber exists?
            logger.verbose(
              `Registered subscriber ${this.id} to event ${event.name}`
            )
            if (cb) {
              return cb(results[1] === 1)
            }
  ...
  removeSubscription(event, cb) {
    logger.verbose(`Removing subscription`)
    return (
      this.redis
        .multi()
        ...
        .exec((err, results) => {
          ...
          if (results[0] != null) {
            ...
            if (wasRemoved) {
              logger.verbose(
                `Subscriber ${this.id} unregistered from event ${event.name}`
              )
            }
            if (cb) {
              return cb(true)
