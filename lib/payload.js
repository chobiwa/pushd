...
class Payload {
  ...
  constructor(data) {
    ...
    for (key of Object.keys(data || {})) {
      ...
      switch (key) {
        ...
        default:
          if (
            (([prefix, subkey] = Array.from((ref = key.split('.', 2)))), ref)
              .length === 2
          ) {
    ...
    let sum = 0
    for (var type of ['title', 'msg', 'data']) {
      sum += (() => {
        const result = []
  ...
  localized(type, lang) {
    ...
    if (this[type][lang] != null) {
      ...
      Payload.prototype.locale_format.test(lang) &&
      this[type][lang.slice(0, 2)] != null
    ) {
  ...
  compile() {
    ...
    for (let type of ['title', 'msg']) {
      for (let lang of Object.keys(this[type] || {})) {
        const msg = this[type][lang]
