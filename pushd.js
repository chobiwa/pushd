...
if (settings.server.redis_socket != null) {
  ...
  settings.server.redis_port != null ||
  settings.server.redis_host != null
) {
  redis = require('redis').createClient(
    settings.server.redis_port,
    settings.server.redis_host
  )
} else {
  redis = require('redis').createClient()
}
  ...
  for (let loggerconfig of Array.from(settings.logging)) {
    ...
    if (transport != null) {
      ...
      process.stderr.write(
        `Invalid logger transport: ${loggerconfig['transport']}\n`
      )
    }
  }
}
...
if (
  (settings.server != null ? settings.server.redis_auth : undefined) != null
) {
...
const tokenResolver = (proto, token, cb) =>
  Subscriber.prototype.getInstanceFromToken(redis, proto, token, cb)
...
const checkUserAndPassword = (username, password) => {
  if ((settings.server != null ? settings.server.auth : undefined) != null) {
    ...
    const passwordOK =
      password != null && password === settings.server.auth[username].password
    if (!passwordOK) {
...
if (settings.server != null ? settings.server.access_log : undefined) {
  app.use(morgan(':method' + ' :url :status'))
}
if (
  (settings.server != null ? settings.server.auth : undefined) != null &&
  (settings.server != null ? settings.server.acl : undefined) == null
) {
...
const testSubscriber = (subscriber) =>
  pushServices.push(
    subscriber,
    null,
    new Payload({ msg: 'Test', 'data.test': 'ok' })
  )
...
const authorize = function (realm) {
  ...
  if ((settings.server != null ? settings.server.auth : undefined) != null) {
    return function (req, res, next) {
      ...
      const allowedRealms =
        (settings.server.auth[req.user] != null
          ? settings.server.auth[req.user].realms
          : undefined) || []
      if (!Array.from(allowedRealms).includes(realm)) {
        logger.error(
          `No access to ${realm} for ${req.user}, allowed: ${allowedRealms}`
        )
        res.status(403).json({ error: 'Unauthorized' })
        return
      }
    ...
    (allow_from = __guard__(
      settings.server != null ? settings.server.acl : undefined,
      (x) => x[realm]
    ))
  ) {
    let network
...
let port =
  __guard__(
    settings != null ? settings.server : undefined,
    (x) => x.tcp_port
  ) != null
    ? __guard__(
        settings != null ? settings.server : undefined,
...
udpApi.on('message', function (msg, rinfo) {
  return zlib.unzip(msg, (err, msg) => {
    ...
    if (!msg) {
      ;[msg, method] = Array.from([method, 'POST'])
    }
    ...
    return this.checkaccess(
      ...
      function () {
        ...
        if (
          (m =
            req.pathname != null ? req.pathname.match(event_route) : undefined)
        ) {
          try {
